...
func TestPlanetGetByID(t *testing.T) {
	...
	useCases := map[string]struct {
		...
		"success": {
			...
			prepare: func(repositoryMock *mocks.MockIPlanetRepository, loggerMock *mocks.MockILoggerProvider) {
				...
				repositoryMock.EXPECT().FindByID(gomock.Any()).Return(expectedSearch, nil)
		...
		"success: should return status 200 if not return documents": {
			...
			prepare: func(repositoryMock *mocks.MockIPlanetRepository, loggerMock *mocks.MockILoggerProvider) {
				...
				repositoryMock.EXPECT().FindByID(gomock.Any()).Return(expectedData, nil)
		...
		"error: should return status code 400 when error to query in database.": {
			...
			prepare: func(repositoryMock *mocks.MockIPlanetRepository, loggerMock *mocks.MockILoggerProvider) {
				...
				repositoryMock.EXPECT().FindByID(gomock.Any()).Return(expectedData, errors.New("error to query name"))
